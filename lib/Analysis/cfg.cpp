...
get_cfg(std::vector<std::pair<std::string, json>> &blockmap) {
  ...
  for (size_t i = 0; i < blockmap.size(); i++) {
    ...
    if (!last.contains("op")) {
      if (i != blockmap.size()) {
        cfg.push_back({name, blockmap[i + 1].first});
      }
    } else if (last.at("op") == "ret") {
      cfg.push_back({name, " "});
      continue;
    } else if (last.at("op") == "jmp") {
      cfg.push_back({name, last.at("labels")[0]});
    } else if (last.back() == "br") {
      cfg.push_back({name, last.at("labels")[0]});
      cfg.push_back({name, last.at("labels")[1]});
    } else {
